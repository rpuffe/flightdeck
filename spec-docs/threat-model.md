...
| Deploy authorization | Each app role is scoped to that app's named resources. Task and execution roles require an app-specific permissions boundary; deploy roles cannot remove it or create arbitrary inline role policies. `iam:PassRole` is limited to the app's exact roles and ECS tasks. | AWS cannot resource-scope task-definition registration or cleanly distinguish one app's rules on the shared ALB listener. Security-group mutation also relies on Flightdeck tags, so this is not a hostile-tenant boundary. |
