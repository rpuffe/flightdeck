...
Application code remains responsible for its own authorization, input
validation, and data handling; authentication is also the application's
responsibility unless it opts into the platform's `auth: cognito` user pool,
and even then session handling and per-route authorization stay in app code.
...
| Runtime isolation | Tasks have no public IP, accept ingress only from the shared ALB, and receive no AWS permissions by default. Optional S3 access is scoped to the app's own environment bucket. Optional secret retrieval is scoped to the ECS execution role's app/environment SSM path; the app task role cannot call SSM. Optional SES sending is scoped to a single From address. All three are capped by the app boundary. | Tasks share a VPC, cluster, ALB, and NAT egress. Outbound traffic is unrestricted through NAT. This is workload separation for a personal platform, not hostile multi-tenant isolation. |
| Public edge | TLS terminates at the shared ALB with host-based routing; services expose only their declared health and application paths through that ALB. | There is no WAF, rate limiting, or DDoS-specific policy beyond AWS defaults. Platform authentication exists only as the opt-in per-app Cognito pool; unauthenticated abuse of public endpoints remains an application and account-cost risk. |
| Outbound mail | `email:` grants `ses:SendEmail`/`SendRawEmail` conditioned on `ses:FromAddress`. The task role's own policy permits exactly one address per environment; the boundary permits the full operator-authorized set for that app, so it caps rather than widens what the role can do. The boundary carries SES only for apps in the operator-controlled `mail_senders` registry, so an app cannot enable its own sending by editing its manifest, and unlisted apps' ceilings are unchanged. Dev is pinned to the platform zone address so test traffic cannot spend a production domain's reputation. The child zone publishes DKIM, SPF `-all`, and DMARC. | SES reputation and sending quota are account-wide, so a listed app sending badly still degrades deliverability for every other app. DMARC starts at `p=none` (report-only). Production domains are verified out of band, so their DNS authentication is not Terraform-enforced. Message content and recipient lists remain entirely the application's responsibility. |
| Secrets and data | Manifest `secrets:` entries are names only. Values are operator-managed SSM Standard `SecureString` parameters, isolated by app/environment and injected by ECS; Terraform state and task definitions contain only ARNs. S3 is private and encrypted, and `s3-retained` adds versioning plus teardown protection. Optional Cognito uses a secretless public PKCE client. | The app deploy role controls its own task definition and can reference any name within its own SSM path; this is honest-workflow isolation, not per-declaration enforcement against a hostile repository. Operator endpoints can still reveal values if used incorrectly, and completed-record retention still needs tested recovery/export procedures. |
