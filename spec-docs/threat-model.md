...
| Secrets and data | The manifest accepts non-secret environment values only. S3 storage, when requested, is private and encrypted at rest with a narrowly scoped task policy. Optional Cognito auth uses a public PKCE client, so no client secret exists anywhere; user credentials live inside AWS's managed service and grant the task role nothing. | There is no secrets injection. The optional bucket and user pool are both teardown-first (`force_destroy` / deletion protection off): app data and user accounts are destroyed with the stack, so neither suits valuable production data without backup, retention, and recovery controls. |
